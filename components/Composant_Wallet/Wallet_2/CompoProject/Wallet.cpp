...
void Wallet::depenser(string publicDestinatire, string publicEmetteur, int montant) {
	if (solde(publicEmetteur) < montant) {
		...
		for (int i = 0; i < blocs.size(); i++) {
			...
			if(MyCompare(publicDestinatire, utxo0.dest)){
				solde += utxo0.montant;
				TXI txi;
				txi.nBloc = i;
				txi.nUtxo = 0;
				string stringdata = to_string(montant) + publicDestinatire + to_string(i) + to_string(0);
				string hash = hasheur.hash(stringdata);//need other composant
				...
				if (solde > montant) {
					...
					for (int k = 0; k < publicDestinatire.length(); k++) newUtxo0.dest[k] = publicDestinatire[k];
					//newUtxo0.dest = uu;

					string stringdata = to_string(montant) + publicDestinatire + to_string(i) + to_string(0);
					string hashTmp = hasheur.hash(stringdata);
					...
					for (int k = 0; k < publicDestinatire.length(); k++) newUtxo1.dest[k] = publicDestinatire[k];
					string stringdatam = to_string(montant) + publicDestinatire + to_string(i) + to_string(1);
					string hashm = hasheur.hash(stringdatam);
					for (int k = 0; k < hashm.length(); k++) newUtxo0.dest[k] = hashm[k];
					tx.UTXOs[1] = newUtxo1;
					Bloc newBloc = mineur.CreerBloc(tx, blocs[blocs.size].hash);//need to be implemented
			...
			//
			if(MyCompare(publicDestinatire, utxo1.dest)) {
				solde += utxo1.montant;
				TXI txi;
				txi.nBloc = i;
				txi.nUtxo = 0;
				...
				//Hash (montant, nBloc, nTX, nUTXO, clé publique)
				string stringdata = to_string(montant) + publicDestinatire + to_string(i) + to_string(0);
				string hash = hasheur.hash(stringdata);//need other composant
				string s = signature.signMessage(hash, privateKeys[i]);
				for(int k = 0; k < s.length(); k++) txi.signature[k] = s[k];
				...
				if (solde > montant) {
					...
					UTXO newUtxo1;
					newUtxo1.montant = solde - montant;
					for (int k = 0; k < publicDestinatire.length(); k++) newUtxo1.hash[k] = publicDestinatire[k];
					string stringdatam = to_string(montant) + publicDestinatire + to_string(i) + to_string(1);
					string hashm = hasheur.hash(stringdatam);
					for (int k = 0; k < hashm.length(); k++) newUtxo1.hash[k] = hashm[k];
					tx.UTXOs[1] = newUtxo1;
					Bloc newBloc = mineur.CreerBloc(tx, blocs[blocs.size].hash);//need other composant
					blocs.push_back(newBloc);
					BCFile.verification();
					BCFile.insert(newBloc);
					break;
